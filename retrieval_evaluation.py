...
# Master switch for whether relevance filtering runs before fusion in this
# eval script, mirroring pipeline.py's GlobalConfig.RELEVANCE_FILTERING_ENABLED.
# Falls back to False if not defined in config.py.
RELEVANCE_FILTERING_ENABLED = getattr(GlobalConfig, "RELEVANCE_FILTERING_ENABLED", False)

# Minimum cosine-similarity relevance score a post-dedup chunk must reach
# against the raw query to survive filtering. Only applied when
# RELEVANCE_FILTERING_ENABLED is True. See pipeline.filter_by_relevance().
# Falls back to 0.0 if not defined in config.py.
RELEVANCE_SCORE_THRESHOLD = getattr(GlobalConfig, "RELEVANCE_SCORE_THRESHOLD", 0.0)
...
def retrieve_all_stages(
    ...
    use_relevance_filtering: bool = RELEVANCE_FILTERING_ENABLED,
    ...
    NOTE ON DEDUPLICATION / RELEVANCE FILTERING: dense_chunks/sparse_chunks
    (used for the standalone "Dense Only"/"Sparse Only" hit-rate metrics)
    are the RAW, unfiltered candidates -- those metrics are meant to
    measure each retriever's independent recall, and applying either dedup
    or relevance filtering to them would make a hit disappear from one
    retriever's score for reasons unrelated to that retriever's own
    behavior. Both dedup (deduplicate_dense_sparse) and relevance
    filtering (filter_by_relevance) are applied only to the copies that
    feed RRF fusion, mirroring pipeline.py's generate_single_response() so
    the Hybrid/Final hit rates reflect what production actually returns.
    Relevance filtering additionally only runs at all when
    use_relevance_filtering is True -- otherwise no extra embedding calls
    are made and the deduped chunks pass through unchanged.
    """
    ...
    # 1.6 Relevancy scoring & filtering before fusion (optional), also
    # mirroring generate_single_response(). When disabled, no extra
    # embedding calls happen and the deduped lists pass through unchanged.
    if use_relevance_filtering:
        embedder = resolve_embedder(vectorstore, EMBEDDING_MODEL)
        filtered_dense_docs, filtered_sparse_docs = filter_by_relevance(
            query=query,
        ...
        filtered_dense_docs, filtered_sparse_docs = deduped_dense_docs, deduped_sparse_docs
    ...
    initial_docs = reciprocal_rank_fusion(
        [filtered_dense_docs, filtered_sparse_docs],
        weights=None,
        top_k=top_k,
    )
...
def fast_evaluate_csv():
    ...
    print(f"[INFO] Relevance filtering: {'ENABLED (threshold=' + f'{RELEVANCE_SCORE_THRESHOLD:.2f}' + ' cosine similarity)' if RELEVANCE_FILTERING_ENABLED else 'DISABLED'}")
    print("-" * 65)
    ...
    for idx, row in enumerate(pbar, start=1):
        ...
        try:
            dense_chunks, sparse_chunks, initial_chunks, final_chunks = retrieve_all_stages(
                ...
                use_relevance_filtering=RELEVANCE_FILTERING_ENABLED,
